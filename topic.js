...
Ext.define("Ext.topic", {
  ...
  publish: function( /* String */ topic, /* Array? */ args) {
    ...
    this.cache[topic] && Ext.each(this.cache[topic], function() {
      ...
      this.callback.apply(this.context || Ext.topic, args || []);
    });
  },

  subscribe: function( /* String */ topic, /* Function */ callback, /* Object? */ context) {
  ...
  unsubscribe: function( /* Array */ handle) {
    ...
    var t = handle[0];
    this.cache[t] && Ext.each(this.cache[t], function(name, index, itSelf) {
      var name = name.callback;
      if (name == handle[1]) {
        this.cache[t].splice(index, 1);
      }
    }, this);
  }
});
